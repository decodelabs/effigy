...
class Mount implements Action
{
    ...
    public function execute(
        ...
        $conf = $this->effigy->project->getLocalManifest()->getRepositoryConfig();
        ...
        foreach ($packages as $name => $package) {
            ...
            if (!isset($conf->{$key})) {
                ...
                if (!$this->effigy->project->run('config', 'repositories.' . $key, '{"type": "path", "url": "' . $path . '", "options": {"symlink": true}}')) {
                    return false;
                }
            }
        }
        ...
        if (
            !empty($requires) &&
            !$this->effigy->project->run(...['require', ...$requires, '--no-update'])
        ) {
            return false;
        }
        ...
        if (
            !empty($devRequires) &&
            !$this->effigy->project->run(...['require', ...$devRequires, '--dev', '--no-update'])
        ) {
            return false;
        }

        $this->effigy->project->run('update');
    ...
    protected function runGlobal(
        ...
        $path = $this->effigy->getGlobalPath();
    ...
    protected function getPath(
        ...
        $this->effigy->config->set('localRepos', [
            $package->name => $path
        ]);
        ...
        if (
            ...
            foreach ($this->roots as $root) {
                if (str_starts_with($root, '/')) {
                    $file = Atlas::getFile($root . '/' . $keyName . '/composer.json');
                } else {
                    $file = $this->effigy->project->rootDir->getFile($root . '/' . $keyName . '/composer.json');
                }
        ...
        return (string)$this->io->ask(
            ...
            validator: function (
                ...
                if (str_starts_with($path, '/')) {
                    $file = Atlas::getFile($path . '/composer.json');
                } else {
                    $file = $this->effigy->project->rootDir->getFile($path . '/composer.json');
                }
