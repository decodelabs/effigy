...
class Unmount implements Action
{
    ...
    public function execute(
        ...
        $this->repositories = $this->effigy->project->getLocalManifest()->getRepositoryConfig();
        ...
        foreach ($packages as $package) {
            $this->effigy->project->run('config', '--unset', 'repositories.local:' . $package->name);
        ...
        if (
            !empty($removes) &&
            !$this->effigy->project->run(...['remove', ...$removes, '--no-update'])
        ) {
            return false;
        }
    ...
    protected function runGlobal(
        ...
        return $this->systemic->run($args, $path);
    }
