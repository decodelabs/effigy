...
class Release implements Action
{
    ...
    public function execute(
        ...
        if (!$this->effigy->run('update-dev-version', $release->version)) {
            return false;
        }
        ...
        if (!$this->effigy->runGit('add', '.')) {
            throw Exceptional::Runtime(
                'Failed to add files to git'
            );
        }

        if (!$this->effigy->runGit('commit', '-m', 'Updated changelog for release ' . $version)) {
            throw Exceptional::Runtime(
        ...
        if (!$this->effigy->runGit('checkout', $releaseBranch)) {
            throw Exceptional::Runtime(
                'Failed to checkout ' . $releaseBranch
            );
        }

        if (!$this->effigy->runGit('merge', $developBranch)) {
            throw Exceptional::Runtime(
        ...
        if (!$this->effigy->runGit('tag', $version, '-m', $body)) {
            throw Exceptional::Runtime(
                'Failed to tag release ' . $version
            );
        }

        if (!$this->effigy->runGit('push', '--all', '--follow-tags')) {
            throw Exceptional::Runtime(
                'Failed to push ' . $releaseBranch
            );
        }

        if (!$this->effigy->runGit('checkout', $developBranch)) {
            throw Exceptional::Runtime(
